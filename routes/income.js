...
router
			...
			.get('/getIncomeFixed/:id', (req, res, next) => {
					...
					db.query(`select amount, description from movements where id_movement_type = 1 && id_user = ?`, id,
			...
			.get('/getIncomeIncidental/:id', (req, res, next) => {
					...
					db.query(`select amount, description from movements where id_movement_type = 2 && id_user = ?`, id,
			...
			.get('/getIncome/:id', function (req, res, next) {
					...
					db.query("Select * from movements where id_user = ? AND id_movement_type = (SELECT id_movement_type WHERE id_movement_type between 1 AND 2);", id, 
			...
			.delete('/deleteItem/:id', (req, res, next) => {
					...
					db.query('delete from movements where id_movement = ?', id_movement, 
